...
pub enum ExpKind<D: Debug + Clone + PartialEq> {
  ...
  Let(
    Vec<(Arc<str>, SourceTrace, VariableKind, Exp<D>)>,
    Box<Exp<D>>,
  ),
  Match(Box<Exp<D>>, Vec<(Exp<D>, Exp<D>)>),
  Block(Vec<Exp<D>>),
  ForLoop {
    increment_variable_name: (Arc<str>, SourceTrace),
...
impl TypedExp {
  pub fn function_from_body_tree(
    ...
    arg_names: Vec<(Arc<str>, SourceTrace)>,
  ...
  fn try_deconstruct_enum_pattern_mut<'a>(
    f: &'a mut Box<TypedExp>,
    args: &'a mut Vec<TypedExp>,
  ) -> Option<(&'a mut Box<FunctionSignature>, &'a mut TypedExp, Arc<str>)> {
  ...
  pub fn try_deconstruct_enum_pattern<'a>(
    f: &'a Box<TypedExp>,
    args: &'a Vec<TypedExp>,
  ) -> Option<(&'a Box<FunctionSignature>, &'a TypedExp, Arc<str>)> {
          ...
          match f_type {
            Type::Function(signature) => {
              if signature
                .abstract_ancestor
                .as_ref()
                .map(|ancestor| ancestor.read().unwrap().associative)
                .unwrap_or(false)
  ...
  pub fn name_or_inner_accessed_name(&self) -> Option<&Arc<str>> {
    let mut exp = self;
    loop {
      match &exp.kind {
      ...
      match &mut exp.kind {
        ...
        Application(f, args) => {
          if let ExpKind::Name(f_name) = &mut f.kind {
            ...
            {
              match &abstract_signature.read().unwrap().implementation {
                    ...
                    .find(|s| s.name.0 == *f_name)
                  {
                        ...
                        .generate_monomorphized(arg_types.clone())
                    {
                      let monomorphized_struct_rc =
                        Arc::new(monomorphized_struct.clone());
                      std::mem::swap(
                        f_name,
                ...
                FunctionImplementationKind::EnumConstructor(variant_name) => {
                    ...
                    })
                  {
                      ...
                      abstract_enum.generate_monomorphized(enum_type.clone())
                    {
                      ...
                      let monomorphized_enum = Arc::new(monomorphized_enum);
                      let mut new_typestate =
  ...
  fn internally_referenced_names(&self) -> Vec<(Arc<str>, Type)> {
    ...
    match &self.kind {
      ...
      Let(items, exp) => {
        let effects =
          items
            .iter()
            .fold(exp.effects(), |mut e, (_, _, _, value_exp)| {
              e.merge(value_exp.effects());
              e
            });
        let bound_names: HashSet<Arc<str>> =
      ...
      ForLoop {
        ...
        effects.merge(continue_condition_expression.effects());
        if let Some(update_expression) = update_expression {
          effects.merge(update_expression.effects());
        }
        effects.merge(body_expression.effects());
        effects
      ...
      Return(exp) => {
        let mut inner_effects = exp.effects();
            ...
            match &mut exp.kind {
              ...
              | Match(_, _) => {
                  ...
                  })
                {
                  ...
                  let mut previously_referenced_names: Vec<(Arc<str>, Type)> =
                    vec![];
              ...
              Block(inner_exps) => {
                ...
                take(inner_exps, |mut inner_exps| {
                  if let Some(last) = inner_exps.pop() {
                    inner_exps
                      .into_iter()
                      .filter(|exp| !exp.effects().is_side_effect_free())
