//! Audio runtime — invoked by `start-audio`.
//!
//! Compiles the program's C backend output to a dylib using `clang`, loads
//! the named function via `libloading`, and starts a `cpal` audio stream
//! that calls it at the audio sample rate.

use std::io::Write;
use std::path::{Path, PathBuf};
use std::process::Command;
use std::sync::{Arc, Mutex};
use std::sync::atomic::{AtomicPtr, AtomicU64, Ordering};

use cpal::SampleRate;
use cpal::traits::{DeviceTrait, HostTrait, StreamTrait};

type AudioFn = unsafe extern "C" fn(f32, f32) -> f32;

/// Active audio thread state. We keep the loaded dylib + cpal stream alive
/// for the lifetime of the process — they're owned by a static slot, since
/// `cpal::Stream` is `!Send` and we can't easily thread it back through.
struct AudioState {
  _lib: libloading::Library,
  _stream: cpal::Stream,
  _counter: Arc<AtomicU64>,
  _fn_ptr: Arc<AtomicPtr<()>>,
}

// SAFETY: AudioState is only stashed in a Mutex on the thread that created
// it; we never actually move it across thread boundaries — the Mutex is just
// a convenient one-time-init slot.
unsafe impl Send for AudioState {}

static AUDIO_STATE: Mutex<Option<AudioState>> = Mutex::new(None);

/// Translate an easl-style function name to its C-emitted form. The C
/// backend replaces `-` with `_`; this mirrors that.
fn easl_fn_name_to_c(name: &str) -> String {
  name.replace('-', "_")
}

/// The easl C backend emits user functions in non-deterministic order
/// without forward declarations, so a function can be called before it's
/// defined and clang rejects it under C99+. Scan for `<type> <name>(<args>) {`
/// lines and prepend matching `<type> <name>(<args>);` decls — duplicate
/// declarations of `print_f32` etc. are legal C as long as they're
/// consistent.
fn inject_forward_decls(c_code: &str) -> String {
  let mut decls = String::new();
  for line in c_code.lines() {
    let trimmed = line.trim_start();
    if trimmed.starts_with("static")
      || trimmed.starts_with("inline")
      || trimmed.starts_with("typedef")
    {
      continue;
    }
    if !line.ends_with('{') {
      continue;
    }
    if !(line.contains('(') && line.contains(')')) {
      continue;
    }
    let decl = format!("{};\n", line.trim_end_matches('{').trim_end());
    decls.push_str(&decl);
  }
  format!("{decls}\n{c_code}")
}

fn compile_c_to_dylib(
  c_code: &str,
  tmp_dir: &Path,
  generation: u64,
) -> Result<PathBuf, String> {
  let c_path = tmp_dir.join(format!("audio_{generation}.c"));
  let dylib_name = if cfg!(target_os = "macos") {
    format!("libaudio_{generation}.dylib")
  } else if cfg!(target_os = "windows") {
    format!("audio_{generation}.dll")
  } else {
    format!("libaudio_{generation}.so")
  };
  let dylib_path = tmp_dir.join(dylib_name);

  {
    let mut f = std::fs::File::create(&c_path)
      .map_err(|e| format!("write C: {e}"))?;
    f.write_all(c_code.as_bytes())
      .map_err(|e| format!("write C: {e}"))?;
  }

  let output = Command::new("clang")
    .args([
      "-shared",
      "-fPIC",
      "-O2",
      "-lm",
      "-o",
      dylib_path.to_str().unwrap(),
      c_path.to_str().unwrap(),
    ])
    .output()
    .map_err(|e| format!("clang exec: {e}"))?;

  if !output.status.success() {
    return Err(format!(
      "clang:\n{}",
      String::from_utf8_lossy(&output.stderr)
    ));
  }

  Ok(dylib_path)
}

fn load_audio_fn(
  dylib_path: &Path,
  symbol_name: &str,
) -> Result<(libloading::Library, AudioFn), String> {
  let lib = unsafe { libloading::Library::new(dylib_path) }
    .map_err(|e| format!("load dylib: {e}"))?;
  let raw_fn = unsafe {
    let sym: libloading::Symbol<AudioFn> = lib
      .get(symbol_name.as_bytes())
      .map_err(|e| format!("symbol `{symbol_name}`: {e}"))?;
    *sym
  };
  Ok((lib, raw_fn))
}

/// Boot the audio thread. Compiles `c_source` to a dylib, loads
/// `entry_name`, and starts a cpal output stream that calls it at the
/// audio sample rate.
///
/// On subsequent calls (e.g. a frame body calls `start-audio` every frame),
/// we hot-swap the loaded function pointer instead of opening a second
/// stream — so repeated calls are cheap and idempotent.
pub fn start_audio_thread(
  entry_name: &str,
  c_source: &str,
) -> Result<(), String> {
  let tmp_dir = std::env::temp_dir().join("easl_audio");
  std::fs::create_dir_all(&tmp_dir)
    .map_err(|e| format!("create tmp dir: {e}"))?;

  let c_symbol = easl_fn_name_to_c(entry_name);
  let c_with_decls = inject_forward_decls(c_source);

  let mut state_lock = AUDIO_STATE.lock().unwrap();

  if let Some(existing) = state_lock.as_ref() {
    // Hot-swap the function pointer in the already-running stream.
    let generation = existing._counter.load(Ordering::Relaxed);
    let dylib_path =
      compile_c_to_dylib(&c_with_decls, &tmp_dir, generation + 1)?;
    let (lib, new_fn) = load_audio_fn(&dylib_path, &c_symbol)?;
    existing
      ._fn_ptr
      .store(new_fn as *mut (), Ordering::Release);
    existing._counter.store(0, Ordering::Relaxed);
    // The old library can't be dropped — code from it may be executing on
    // the audio thread.  We leak it intentionally; the stream's lifetime
    // is the lifetime of the process anyway.
    Box::leak(Box::new(lib));
    return Ok(());
  }

  let dylib_path = compile_c_to_dylib(&c_with_decls, &tmp_dir, 0)?;
  let (lib, raw_fn) = load_audio_fn(&dylib_path, &c_symbol)?;

  let host = cpal::default_host();
  let device = host
    .default_output_device()
    .ok_or_else(|| "no audio output device".to_string())?;

  let sample_rate = 44_100u32;

  let config = device
    .supported_output_configs()
    .map_err(|e| format!("query audio configs: {e}"))?
    .find(|c| {
      c.sample_format() == cpal::SampleFormat::F32
        && c.min_sample_rate() <= SampleRate(sample_rate)
        && c.max_sample_rate() >= SampleRate(sample_rate)
    })
    .ok_or_else(|| "no suitable f32@44.1kHz config".to_string())?
    .with_sample_rate(SampleRate(sample_rate));

  let channels = config.channels() as usize;

  let fn_ptr: Arc<AtomicPtr<()>> = Arc::new(AtomicPtr::new(raw_fn as *mut ()));
  let counter = Arc::new(AtomicU64::new(0));

  let cb_counter = counter.clone();
  let cb_fn_ptr = fn_ptr.clone();

  let stream = device
    .build_output_stream(
      &config.into(),
      move |output: &mut [f32], _: &cpal::OutputCallbackInfo| {
        let fp = cb_fn_ptr.load(Ordering::Acquire);
        let func: AudioFn = unsafe { std::mem::transmute(fp) };
        let rate = sample_rate as f32;
        for frame in output.chunks_mut(channels) {
          let n = cb_counter.fetch_add(1, Ordering::Relaxed);
          let t = n as f32 / rate;
          let sample = (unsafe { func(t, rate) }).clamp(-1.0, 1.0);
          for s in frame.iter_mut() {
            *s = sample;
          }
        }
      },
      |err| eprintln!("audio stream error: {err}"),
      None,
    )
    .map_err(|e| format!("build audio stream: {e}"))?;

  stream.play().map_err(|e| format!("start audio playback: {e}"))?;

  *state_lock = Some(AudioState {
    _lib: lib,
    _stream: stream,
    _counter: counter,
    _fn_ptr: fn_ptr,
  });

  Ok(())
}
