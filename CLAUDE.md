...
- **Each capture is lifted to an ordinary storage-write global** (elided binding numbers) named `<scope-struct>_audio_data_<capture>`. Captured *closures* recurse (`cloneify_captured_closure` with `ClosureLiftTarget::Audio` — shared with the GPU dispatch lift; the target enum parameterizes capture global kind, naming, and capture-type rules), so arbitrary capture depth works. The closure itself gets a scope-less **audio clone** named `{original}_audio` — deterministic, so both runtimes can derive the entry name from the closure's abstract ancestor at `start-audio` time — with the trailing scope param dropped and `scope.field` accesses rewritten to the lifted globals. Entry marking moves from the original to the clone; the original stays intact for direct CPU calls. Capture types that can't cross the thread boundary are rejected at lift time: `UnshareableAudioCapture` for String-containing types and for types that *embed* a runtime-sized array without being one (a direct `[T]` capture is fine — it lifts to an ordinary dyn global).
