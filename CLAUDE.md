...
- `(audio-time)` — the global audio clock (`t`), callable **only from audio code**: `validate_audio_info_usage` walks reachability from every non-audio entry root (same function-reference-following walk as `thread_shared_globals`, including the `start-audio` argument-edge cut) and rejects reachable calls with `AudioInfoOutsideAudio`. Known accepted hole: an `audio-time` call in a closure-constructor body sits inside the cut, so it isn't flagged and reads main's zeroed copy.
