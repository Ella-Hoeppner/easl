...
When an easl program calls `(start-audio audio-fn)`, the runtime starts a cpal output stream that calls `audio-fn` once per sample. The full contract is `(t: f32, rate: f32) -> f32`, but the arity is relaxed: 0-, 1-, and 2-arg functions are all valid audio entries (all args must be `f32`); a 1-arg entry receives only `t`, a 0-arg entry receives nothing (`VmAudioDriver` writes `t`/`rate` into the entry's arg slots conditionally, guided by `Function::arg_words` — the summed arg slot count recorded on every VM function). Stateful entries — closures with captured scope — are also valid; see "Stateful closure audio entries" below. There are two backends, selected via `AudioBackend`:
...
- `AudioBackend::C` (gated on the `c_audio` feature): JIT-compile the program's C-backend output to a dylib via `clang`, dlopen it via `libloading`, and have the cpal callback call the loaded function pointer. Faster but requires `clang` on the host. Selecting `AudioBackend::C` without `c_audio` enabled panics at audio-compile time. The C driver calls the entry through a fixed `fn(float, float) -> float` pointer, so it supports only exact 2-arg entries — `try_compile_audio_source` panics with a clear message if a relaxed-arity or closure entry is compiled for the C backend.
