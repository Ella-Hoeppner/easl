...
- **Known gaps** (panic with clear messages): strings exist only as print/key-query literals, and nested `spawn-window` is rejected. (Whole dynamic arrays passing by value / array-to-array assignment now work — see "First-class runtime-sized arrays".)
...
- Printing a dyn *local* directly is a clear compile panic (print an element or a global instead); whole-global printing works as before.
