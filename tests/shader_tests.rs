...
success_test!(matrix);
success_test!(index_vec);
success_test!(index_mat_column);
success_test!(index_mat_assign);
...
success_test!(audio_storage_ref_helper_not_emitted);
...
error_test!(
  external_local_var_failure,
  CompileErrorKind::ExternalLocalVar
);
error_test!(
